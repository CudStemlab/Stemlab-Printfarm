// 3MF helpers specific to the slicer-proxy upload path: plate G-code extraction,
// Klipper thumbnail synthesis, and the Bambu toolhead-camera patch.
//
// The ZIP entry reader and the slice_info.config parsers it needs live in
// ../server/threemf.js — server/ is copied into this image (see
// Dockerfile.slicer-proxy) and is the only directory the web tier can also
// import from, so there is exactly one 3MF reader in the tree. They are
// re-exported below so this module stays the single import surface for
...
import {
  ...
  readZipEntry,
} from '../server/threemf.js';

export { extractFilamentGramsFrom3mf, readZipEntry };
