...
6000, Moonraker HTTP), ESP32 status-light MQTT (embedded broker, wss-only —
WebSocket `/mqtt` through nginx TLS, no raw-TCP listener or extra host port —
shared LAN credential, status-only topics).
